...
const ImageSlider = ({urls}: ImageSliderProps) => {
  ...
  return (
    <div className="group relative bg-zinc-100 aspect-square overflow-hidden rounded-xl">
        ...
        <Swiper spaceBetween={50} slidesPerView={1} modules={[Pagination]} onSwiper={(swiper) => setSwiper(swiper)} className='h-full w-full'>
            {urls.map((url, i) => (
                <SwiperSlide key={i} className='-z-10 relative w-full h-full'>
                    <Image fill loading='eager' className='-z-10 h-full w-full object-cover object-center' src={url} alt='Product Images' />
                </SwiperSlide>
            ))}
        </Swiper>
    </div>
